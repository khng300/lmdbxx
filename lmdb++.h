...
namespace lmdb {
  ...
  static inline MDB_cursor *cursor_dup(MDB_cursor* cursor);
...
lmdb::cursor_dup(MDB_cursor* cursor) {
  MDB_cursor *result;
  int rc = ::mdb_cursor_dup(cursor, &result);
  if (rc != MDB_SUCCESS) {
    error::raise("mdb_cursor_dup", rc);
  }
  return result;
}
...
public:
  ...
  /**
   * @return map address and map size
   */
  std::string_view get_map() {
    if (sizeof(int) != 4 || sizeof(long) != 8 || sizeof(void*) != 8) error::raise("get_internal_map: only LP64 supported", 0);

    MDB_envinfo2 arg;
    lmdb::env_info2(_handle, &arg);
    return std::string_view((char *)arg.me_map, arg.me_mapsize);
  }
};
...
public:
  ...
  cursor(const cursor& other) noexcept {
    if (other._handle != nullptr) {
      _handle = cursor_dup(other._handle);
    }
  }
